const courseDB = {
  ...
  "general": {
    ...
    "校核心必修": [{n:"體育（一）",c:1},{n:"體育（二）",c:1},{n:"英文（一）",c:2},{n:"英文（二）",c:2},{n:"中文閱讀與書寫（一）",c:2},{n:"中文閱讀與書寫（二）",c:2}]
  }
};
...
function toggleMode(mode) {
    const listSec = document.getElementById('list-mode-section');
...
function addCourse() {
    ...
    if (obj.main === 'major_auto') {
        obj.main = obj.sub.includes("必修") ? "req" : "ele";
    }
    obj.id = Date.now();
    selected.push(obj);
    save(); render();
...
function addManualCourse() {
    ...
    if (categoryRaw.startsWith("gen-")) {
        mainCat = "gen";
        subCat = categoryRaw.replace("gen-", "");
    } else {
        mainCat = categoryRaw;
        subCat = "手動新增";
    }
...
function render() {
    ...
    const semLabels = { "1-1":"大一上", "1-2":"大一下", "2-1":"大二上", "2-2":"大二下", "3-1":"大三上", "3-2":"大三下", "4-1":"大四上", "4-2":"大四下", "general":"通識/核心", "manual":"手動輸入課程" };
    
    Object.keys(semLabels).forEach(semKey => {
        const courses = selected.filter(c => c.sem === semKey);
        if (courses.length > 0) {
            const block = document.createElement("div");
            block.className = "semester-block";
            const ulId = `ul-${semKey.replace(/-/g, '')}`;
            block.innerHTML = `<div class="semester-title">${semLabels[semKey]}</div><ul id="${ulId}"></ul>`;
    ...
    if (eleTotal > 22) { stats.ele = 22; stats.free += (eleTotal - 22); } else { stats.ele = eleTotal; }
...
function save() { localStorage.setItem("nua_data_final", JSON.stringify(selected)); }
function load() { const d = localStorage.getItem("nua_data_final"); if (d) selected = JSON.parse(d); }
